...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Fungsi untuk memproses konten agar aman dan terformat
  const renderContent = () => {
    let content = post.content || "";

    // 1. Preserve Line Breaks (Penting untuk copas)
    if (!content.includes("<p>") && !content.includes("<br") && !content.includes("<div>")) {
       content = content.replace(/\n/g, "<br/>");
    }

    // 2. Basic Markdown Support (Bold & Italic) if not HTML
  ...
  return (
    <div className="bg-slate-950 min-h-screen text-slate-200 selection:bg-cyan-500/30">
      ...
      <div className="relative pt-32 pb-20 overflow-hidden">
         {/* Background Elements - Dibuat lebih redup agar tidak mengganggu */}
         ...
         <article className="container mx-auto px-4 max-w-4xl relative z-10">
            ...
            {/* Content Container - Memberikan background panel agar teks terbaca jelas */}
            <div className="relative bg-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-3xl p-6 md:p-10 shadow-2xl">
                
                {/* Content Body - TIPOGRAFI YANG DIPERBAIKI UNTUK KENYAMANAN BACA */}
                <div className="prose prose-lg prose-invert mx-auto max-w-none
                  /* RESET GAYA BAWAAN (Google Docs/Word) */
                  [&_*]:!bg-transparent 
                  [&_*]:!font-sans
                  
                  /* MEMAKSA WARNA TEKS (Override inline styles color:black) */
                  [&_p]:!text-slate-300 [&_p>span]:!text-slate-300
                  [&_li]:!text-slate-300 [&_li>span]:!text-slate-300
                  [&_h1]:!text-white [&_h1>span]:!text-white
                  [&_h2]:!text-white [&_h2>span]:!text-white
                  [&_h3]:!text-cyan-100 [&_h3>span]:!text-cyan-100
                  [&_strong]:!text-white [&_strong>span]:!text-white
                  [&_b]:!text-white [&_b>span]:!text-white

                  /* UKURAN FONT & SPASI */
                  [&_p]:!text-lg [&_p]:md:!text-xl 
                  [&_p]:!leading-8  
                  [&_p]:!mb-8       
                  
                  [&_li]:!text-lg [&_li]:md:!text-xl
                  [&_li]:!mb-2
