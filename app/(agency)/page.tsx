...
const CyberFrog = () => {
  ...
  return (
    <motion.div
      ...
      // 2. GLOBAL FLOATING ANIMATION (The whole frog moves)
      animate={{ 
        y: [0, -20, 0],
      }}
      transition={{ 
    ...
    >
      ...
      >
        ...
        {/* HEAD */}
        <circle cx="32" cy="50" r="16" fill="#22c55e" /> {/* Left Eye bump */}
        <circle cx="68" cy="50" r="16" fill="#22c55e" /> {/* Right Eye bump */}
        <ellipse cx="50" cy="55" rx="20" ry="18" fill="#22c55e" /> {/* Head base */}
        ...
        <motion.path
          d="M20 50 Q32 38 44 50" // Curved eyelid shape
          stroke="#166534"
          strokeWidth="12" // Thick stroke covers the eye
          strokeLinecap="round"
          fill="#166534"
          initial={{ opacity: 0 }}
          animate={{ 
            // When blinking, opacity goes to 1, effectively "closing" the eye
...
const FrogGatekeeper = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const handleBugClick = () => {
    ...
    // Animate bug moving to mouth (center)
    setTimeout(() => {
      setBugPosition({ x: 50, y: 50 });
      setIsSwallowing(true);
      
      // Close mouth and show access granted
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white overflow-x-hidden">
      ...
      <section id="home" className="relative min-h-screen flex items-center justify-center pt-32 pb-32 overflow-hidden z-10 bg-slate-950">
        ...
        {/* Cyber Frog - Interactive Element */}
        <CyberFrog />
